#module to visualize the generated data
...
class Visualizer:
    ...
    def plot_load_vs_capacity(self):
        #Extract load and capacity data for the first 100 time intervals
        load = np.array(self.node.load_history[:100],dtype=np.float32)
        capacity =  np.array(self.node.capacity_history[:100],dtype=np.float32)
        fig = plt.figure() 
        # Create and save the load vs capacity plot
        ...
        plt.legend()
        plt.savefig(f'Load_vs_Capacity{self.model_name}.png')
        plt.close()
    ...
    def plot_underuse_overuse(self):
        ...
        capacity =  np.array(self.node.capacity_history[:100],dtype=np.float32)
        fig = plt.figure() 
        #calculate overuse_underuse values
        y = load - capacity 

        # Create and save the underuse-overuse plot
        ...
        plt.ylabel('Computation In Flops')
        plt.legend()
        plt.savefig(f'Overused_vs_Underused_{self.model_name}.png')
        plt.close()
    ...
    def plot_combined_error(self):
        ...
        plt.legend()
        plt.savefig(f'error_bar_chart.png')
        plt.close()
    ...
    def plot_delayed_tasks(self):
        ...
        plt.legend()
        plt.savefig(f'delay_bar_chart.png')
        plt.close()

        self.plot_and_upload(fig, 'delay_bar_chart.png',)

    def plot_all(self):
        self.plot_load_vs_capacity()
        self.plot_underuse_overuse()
        self.plot_combined_error()
        self.plot_delayed_tasks()
